...
import {getStore} from '@/utils/commons' //缓存
...
const mapStateToProps = (state) => {/*将需要的state的节点注入到与此视图数据相关的组件（props）上.也即用于建立 State 到 Props 的映射关系*/
  return {
    userInfo: state.userInfo
  }
}

const mapDispatchToProps = (dispatch) => {//将需要绑定的响应事件注入到组件上（props上）。
  return {
    //dispatch在action.js里的saveUserInfo,设置redux全局userInfo
    saveUserInfo: (userInfo) => dispatch(saveUserInfo(userInfo))//把saveUserInfo注入到Props上
  }
}
